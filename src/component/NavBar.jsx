...
const user = {
    ...
    const nav = [
        ...
        { name: 'New Account', href: '/new', current: false },
...
function NavBar() {
  ...
  useEffect(() => {
    setNavigation(nav => {
       const up = nav.map(x => {
        if(pathname === x.href) return({...x, current:true})
        else return ({...x, current:false})
       });
       return up
    })
  },[pathname])
    ...
    <div className="w-full fixed m-0 z-10">
                ...
                <div className="flex h-16 items-center justify-between">
                  ...
                  <div className="hidden md:block">
                    <div className="ml-4 flex items-center md:ml-6">
                      <button
                        type="button"
                        className="relative rounded-full bg-yellow-600 p-1 text-yellow-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800"
                      >
        ...
        {/* <header className="bg-white shadow">
          <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">{navigation.find(x => x.current).name}</h1>
          </div>
        </header> */}
        </div>
      </div>
